...
// 环境变量
const UUID = process.env.UUID || '03e46db9-8bd6-4dcc-abac-6203af25a4c1'; // 使用哪吒v1，在不同的平台部署需修改UUID，否则会覆盖
...
const NEZHA_KEY = process.env.NEZHA_KEY || '7Ner02g5Khs71ghjlSZGfjKjdFilje5K';             // v1的NZ_CLIENT_SECRET或v0的agent端口  
const AUTO_ACCESS = process.env.AUTO_ACCESS || ture;      // 是否开启自动访问保活,false为关闭,true为开启,需同时填写DOMAIN变量
const XPATH = process.env.XPATH || UUID.slice(0, 8);       // xhttp路径,自动获取uuid前8位
const SUB_PATH = process.env.SUB_PATH || `${UUID}`;        // 节点订阅路径,默认位uuid
const DOMAIN = process.env.DOMAIN || 'https://qwe8899-x.hf.space';                   // 域名或ip,留空将自动获取服务器ip
